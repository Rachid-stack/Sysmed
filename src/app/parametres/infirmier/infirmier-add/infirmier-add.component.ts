...
@Component({
  selector: 'app-infirmier-add',
  templateUrl: './infirmier-add.component.html',
  styleUrls: ['./infirmier-add.component.css']
})
export class InfirmierAddComponent implements OnInit {
  @ViewChild(PersonneChooseComponent)
  private personneComponent: PersonneChooseComponent;
  url = 'http://localhost:3000/api/uploads/';
  id = null;
  infirmierForm: FormGroup;
  modalReference: any = {};
  ...
  constructor(private http: HttpClient,
              ...
              private formBuilder: FormBuilder) { this.infirmierForm = this.formBuilder.group({ }); }
  ...
  ngOnInit() {
    ...
    this.newInfirmier = { code_inf: '', id_titre: '', id_service: '' };
  ...
  saveInfirmier(id){
    this.newInfirmier.id_personne = id;
    this.api.ajouter('infirmier', this.newInfirmier).subscribe((res) => {
      this.newInfirmier = {};
      console.log(res);
    });
    this.closeModal();
  ...
  open(content) {
    ...
    this.modalReference = this.modalInfirmier.open(content, this.ngbModalOptions);
  }
  ...
  ajouterPersonnel() {
    ...
    if(!this.isNew && this.id_personne != ''){
      this.saveInfirmier(this.id_personne);
    }else{
